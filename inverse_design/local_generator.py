...
# %% auto 0
__all__ = ['debug', 'dilate', 'GeneratorState', 'Times', 'log', 'valid_touches', 'possible_pixels', 'required_pixels',
           'resolving_touches', 'resolving_touches_no_dilate', 'free_touches', 'free_touches_no_dilate',
           'select_single', 'touch', 'local_dilate', 'track', 'update_resolving', 'generate',
...
def generate(heatmap, brush):
  #global state
  state = GeneratorState(heatmap, brush) # Can have the brushes pregenerated
  global times
  ...
  if debug>0:
    ...
    plt.imshow(state.p_s_existing)
    plt.subplot(222)
    plt.title("Solid and Void Touches")
    plt.imshow(state.t_s.astype(int)-state.t_v)
    plt.subplot(223)
    plt.imshow(heatmap)
...
# %% ../notebooks/09_local_generator.ipynb 21
